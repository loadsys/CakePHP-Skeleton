...
exec { 'dotfiles':
  ...
  command => "cp -r /vagrant/Lib/puphpet/files/dot/.[a-zA-Z0-9]* /home/${::ssh_username}/ \
              && chown -R ${::ssh_username} /home/${::ssh_username}/.[a-zA-Z0-9]* \
              && cp -r /vagrant/Lib/puphpet/files/dot/.[a-zA-Z0-9]* /root/",
  onlyif  => 'test -d /vagrant/Lib/puphpet/files/dot',
  returns => [0, 1],
...
if $yaml_values == undef {
  $yaml_values = loadyaml('/vagrant/Lib/puphpet/config.yaml')
...
if hash_key_equals($php_values, 'install', 1) {
  ...
  if count($php_values['modules']['pear']) > 0 {
    # Set channels to auto discover, update them, and respect explicit repositories if named. -bp
...
define php_pear_mod {
  if ! defined(Php::Pear::Module[$name]) {
    # Use the repository value, if explicitly set. -bp
    if is_hash($name) {
