...
if [[ (${#files[@]} -gt 0) ]]; then
	...
	ONCE_DIR="$( cd -P "$( dirname "${VAGRANT_CORE_FOLDER}/files/${EXEC_ONCE_DIR}/./" )" >/dev/null 2>&1 && pwd )";
    find "${ONCE_DIR}" -maxdepth 1 -not -path '*/\.*' -type f \( ! -iname "empty" \) -exec cp -n '{}' "/.puphpet-stuff/${EXEC_ONCE_DIR}-ran" \;
...
ALWAYS_DIR="$( cd -P "$( dirname "${VAGRANT_CORE_FOLDER}/files/${EXEC_ALWAYS_DIR}/./" )" >/dev/null 2>&1 && pwd )";
find "${ALWAYS_DIR}" -maxdepth 1 -not -path '*/\.*' -type f \( ! -iname "empty" \) -exec chmod +x '{}' \; -exec {} \;
